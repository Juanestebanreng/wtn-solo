...
function filterLabel(f: Filter) {
  return { all: 'All time', today: 'Today', week: 'This week', month: 'This month', year: 'This year' }[f];
}
...
export default function NotesClient({
  ...
  const filtered = wtns.filter(w => {
    if (w.status !== 'final') return false;
    ...
    <div>
      ...
      {selected.size > 0 && (
        <div className="mt-3 flex flex-wrap items-center gap-3 rounded border border-steel bg-white px-4 py-3 text-sm">
          ...
          <button onClick={() => setShowEmailBar(true)} className="rounded border border-steel px-3 py-1.5 text-xs font-medium text-ink hover:bg-paper2">
            Send by email
          </button>
      ...
      {showEmailBar && (
        <div className="mt-2 flex flex-wrap items-center gap-3 rounded border border-steel bg-white px-4 py-3">
          <input
            ...
            className="flex-1 rounded border border-steel px-3 py-1.5 text-sm text-ink focus:border-ink focus:outline-none min-w-0"
          />
          <button
            onClick={handleSend}
            disabled={sending || !email}
            className={`rounded px-4 py-1.5 text-sm font-medium text-white disabled:opacity-50 ${sending ? 'bg-slate' : 'bg-ink hover:bg-ink/90'}`}
          >
            {sending ? 'Sending…' : `Send ${selected.size} PDF${selected.size > 1 ? 's' : ''}`}
          </button>
          <button onClick={() => setShowEmailBar(false)} className="text-xs text-slate underline">Cancel</button>
      ...
      <div className="mt-3 divide-y divide-steel rounded border border-steel bg-white">
        ...
        {filtered.map(w => {
          ...
          return (
            <div key={w.id} className={`flex items-center gap-3 px-4 py-3 text-sm ${isSelected ? 'bg-amber/5' : 'hover:bg-paper2'}`}>
              <input
                type="checkbox"
                checked={isSelected}
              ...
              <Link href={`/dashboard/wtns/${w.id}`} className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 truncate text-slate">
                  ...
                  {w.waste_description && <span> — {w.waste_description}</span>}
                </p>
              </Link>
